...
const GROUNDING_RULES = [
  ...
  // أضيفت بعد جولة QA 14 يوليو: سؤال خارج النطاق («متى الساعة؟» في متجر نقل) دفع النموذج لكتابة
  // «أحتاج أحوّل طلبك للفريق»، فصعّده حارس التحويل تحويلاً بشرياً حقيقياً — هدرٌ لانتباه موظف على
  // سؤال تافه. القاعدة تعطي المخرج الصحيح (رفض لطيف + إعادة توجيه) بدل التهرّب بالتحويل.
  "قاعدة نطاق المتجر: أنت وكيل هذا النشاط التجاري وحده. إن سأل العميل شيئاً خارج نطاق المتجر ومنتجاته وخدماته وسياساته (كالوقت أو الطقس أو الأخبار أو معلومة عامة أو رأي شخصي أو دردشة لا تخص المتجر)، فاعتذر بلطف وبإيجاز أنه خارج اختصاصك وأعِد توجيهه لما يخصّ المتجر — ولا تعرض التحويل لموظف ولا تَعِد به من أجل سؤال خارج النطاق (التحويل البشري لمسائل المتجر وحدها؛ ويبقى قائماً فقط إن طلب العميلُ نفسُه موظفاً بشرياً صراحةً).",
