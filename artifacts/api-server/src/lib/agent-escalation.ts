...
// ─── تصعيد مبني على نيّة العميل (الحل الجذري، 13 يوليو 2026) ──────────────────────
// الدرس المثبت حيّاً: مطاردة صياغات الوكيل اللانهائية (وعود التحويل/الاسترجاع/التصعيد) خاسرة —
// النموذج يجد صيغة جديدة كل مرة (مورفولوجيا، غائب/متكلم، بادئات). الحل الأعلى ارتفاعاً: نفحص نيّة
// **العميل** في رسالته الواردة (مجموعة محدودة) لا صياغة الوكيل — فنتصرّف مهما كتب الوكيل.
// هذا يكمّل حُرّاس الردّ (أ-2) لا يستبدلها: دفاعٌ بعمق (النيّة تُنبّه/تصعّد، وحُرّاس الردّ تمنع الوعد).

// (1) مطالبة استرجاع مال العميل — الوكيل لا يملك أداة استرداد إطلاقاً، فهي دائماً بحاجة بشر.
// المرساة: إشارة مالٍ يملكه العميل (فلوسي/مبلغي/دفعته) + نيّة إرجاع/مطالبة. لا يلتقط سؤال السياسة
// المجرّد («هل عندكم استرجاع؟» — لا «فلوسي/مبلغي») فيبقى الوكيل يجيبه من المعرفة. المعالجة **ناعمة**:
// التاجر يُشعَر بكل رسالة استرجاع (لا يعتمد على صياغة الوكيل)، والوكيل يواصل خدمة أسئلة السياسة.
const REFUND_MONEY_REF = /فلوسي|مبلغي|نقودي|دراهمي|مبالغي|دفعته|دفعتها|اللي دفعت|المبلغ المدفوع/;
// تُكتب الأنماط مطبَّعة مسبقاً (normalizeArabic يحوّل ى→ي): «أبغى» تصبح «ابغي» في المدخل.
const REFUND_INTENT_CUE = /ابغي|ابي|بدي|عايز|اريد|رجعو|ارجعو|ترجعو|ترجعون|استرجاع|استرداد|استرد|استعيد|وين|فين/;

export function detectsRefundDemand(text: string): boolean {
  const normalized = normalizeArabic(text);
  return REFUND_MONEY_REF.test(normalized) && REFUND_INTENT_CUE.test(normalized);
}

// (2) نزاع جدّي/تهديد قانوني/اتهام احتيال — يحتاج بشراً فوراً (سمعة/قانون). أنماط ضيّقة صريحة
// (لا «المنتج تالف» العادية القابلة للمعالجة بالاستبدال). المعالجة **صلبة**: تحويل فوري للبشر.
// «نصب» المجرّدة مستبعدة عمداً (تلتبس بـ«منصب») — نكتفي بالصيغ التي لا تلتبس.
const SERIOUS_DISPUTE_PATTERNS = [
  "احتيال",
  "محتال",
  "محتالين",
  "نصابين",
...
export function detectsSeriousDispute(text: string): boolean {
  const normalized = normalizeArabic(text);
