...
export async function consumeInventoryForOrder(input: InventoryOperationContext) {
  ...
  try {
    ...
    if (existingMovement.rowCount) {
      ...
      return { idempotent: true, consumed: 0 };
    }
    ...
    for (const reservation of reservations.rows) {
      ...
      if (!stock || stock.on_hand < reservation.quantity || stock.reserved < reservation.quantity) {
...
export async function returnInventoryForOrder(input: InventoryOperationContext) {
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    ...
    await client.query("COMMIT");
    return { returned: consumed.rows.length };
  } catch (error) {
    await client.query("ROLLBACK");
    throw error;
  } finally {
