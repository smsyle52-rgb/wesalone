...
router.get("/meta/embedded-signup/callback", async (req: Request, res: Response): Promise<void> => {
  ...
  if (attempt) {
    ...
    try {
      ...
      // ربط تلقائي دائماً لأول رقم مكتشف — حتى مع عدة WABAs (اكتُشف 12 يوليو أن للتاجر حسابين،
      // فذهب لصفحة الاختيار التي تحجبها بوابة onboarding لمستخدم أول مرة فعلق نهائياً). كل الأرقام
      // المكتشفة تخص حساب ميتا الذي سجّل الدخول به التاجر نفسه، فربط أولها آمن؛ يضيف البقية لاحقاً
      // من صفحة التكاملات. هذا يُلغي اعتماد مسار الجوال على صفحة الاختيار الهشّة كلياً.
      // distinctWabaCount سُجّل أعلاه للرؤية فقط. mobileSelectedFirstPhone دائماً:
