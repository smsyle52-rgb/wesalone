...
async function jsonBody(response: globalThis.Response): Promise<Record<string, unknown>> {
...
describe("WhatsApp Business Profile routes", () => {
  ...
  it("rejects an unverified email before entering the feature router", async () => {
    ...
    expect((await jsonBody(response)).code).toBe("EMAIL_NOT_VERIFIED");
  });
  ...
  it("requires integrations:read for GET", async () => {
    ...
    expect(response.status).toBe(403);
    expect((await jsonBody(response)).requiredPermission).toBe("integrations:read");
  });
  ...
  it("requires integrations:update for PATCH", async () => {
    ...
    expect(response.status).toBe(403);
    expect((await jsonBody(response)).requiredPermission).toBe("integrations:update");
  });
  ...
  it("passes only activeWorkspaceId to the service and rejects another workspace", async () => {
    ...
    expect((await jsonBody(response)).code).toBe("WHATSAPP_ACCOUNT_NOT_FOUND");
  ...
  it("returns a clear inactive-account error", async () => {
    ...
    expect((await jsonBody(response)).code).toBe("WHATSAPP_ACCOUNT_INACTIVE");
  });
  ...
  it("updates the profile successfully through PATCH", async () => {
    ...
    expect((await jsonBody(response)).source).toBe("meta");
  ...
  it("does not convert a Meta failure into success", async () => {
    ...
    expect((await jsonBody(response)).code).toBe("META_BUSINESS_PROFILE_ERROR");
  });
  ...
  it("rejects an unsupported profile image MIME type", async () => {
    ...
    expect((await jsonBody(response)).code).toBe("PROFILE_IMAGE_MIME_NOT_ALLOWED");
  });
  ...
  it("rejects a profile image larger than the configured limit", async () => {
    const response = await request(
      ...
      `/api/whatsapp-management/accounts/${ACCOUNT_ID}/business-profile/photo`,
      {
        ...
        body: Buffer.alloc(BUSINESS_PROFILE_IMAGE_MAX_BYTES + 1),
      },
    );
    expect(response.status).toBe(413);
    expect((await jsonBody(response)).code).toBe("PROFILE_IMAGE_TOO_LARGE");
  });
