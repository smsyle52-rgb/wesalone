      ...
      render: (row: AutomationRow) => (
        <details className="relative">
          ...
          <div className="absolute left-0 z-10 mt-1 w-44 rounded-lg border border-border bg-popover p-1 shadow-lg">
            <button className="block w-full rounded px-3 py-2 text-right text-sm hover:bg-muted" onClick={() => setLocation(`/automations/${row.id}`)}>
              {t("common.edit")}
            </button>
            {canActivate && row.status !== "active" && (
              <button className="block w-full rounded px-3 py-2 text-right text-sm hover:bg-muted" onClick={() => actionMutation.mutate({ id: row.id, action: "activate" })}>
                {t("common.activate")}
              </button>
            )}
            {canActivate && row.status === "active" && (
              <button className="block w-full rounded px-3 py-2 text-right text-sm hover:bg-muted" onClick={() => actionMutation.mutate({ id: row.id, action: "pause" })}>
                {t("common.pause")}
              </button>
            )}
            {canDelete && (
