...
آخر تحديث: 4 يوليو 2026 (Claude — **عطل طارئ حيّ مُصلَح:** عميل ربط رقم واتساب فعلياً لكنه لم يظهر في لوحة التحكم — السبب: `GET /integrations/meta/channels` (نفس الاستدعاء الذي تعتمد عليه صفحة التكاملات لعرض القنوات المربوطة) كان يفشل بـ500 لأن عمود `channel_accounts.default_agent_id` موجود في سكيمة Drizzle لكنه غير موجود فعلياً — لم يُنقل قط إلى `migrate-phase345.sql`. الربط نفسه كان ينجح (الكتابة لا تحتاج هذا العمود)، لكن أي قراءة لاحقة للقناة كانت تفشل صامتاً خلف رسالة "حدث خطأ غير متوقع". أُضيف العمود بأسلوب idempotent مطابق للنمط الحالي، وتحقّقتُ محلياً أن `200` تحلّ محل `500`. + من نفس الفحص: بوابة `requireVerifiedEmail` تحجب onboarding عن أي تسجيل بكلمة مرور غير مؤكَّد بريده (صفحة onboarding الوحيدة المعروضة خارج `<Layout>` فلا ترى شاشة التحقق الجاهزة أصلاً) — أُضيفت + اكتُشف أن `/auth/me` كان لا يُحدِّث `emailVerified` داخل الجلسة نفسها بعد التحقق (عطل جلسة منفصل، مُصلَح) + عطل تكدّس أزرار الجوال في تذييل خطوات onboarding الثلاث. جاهز للدفع الطارئ)
