...
آخر تحديث: 4 يوليو 2026 (Claude — **حادثة عميل حقيقي فقدناه أثناء onboarding — جذر السبب مؤكَّد من إنتاج مباشرة:** عميل وافق فعلياً على ربط واتساب من نافذة Meta (يظهر "وصال ون" في قسم التطبيقات المرتبطة لديه) لكن `channel_accounts` عندنا **صفر صفوف** له — التحقق المباشر من قاعدة الإنتاج (Cloud Shell + cloud-sql-proxy، محلي فشل بـ invalid_rapt) أثبت هذا قطعياً. السبب الأرجح: `waitForCapturedSignupInfo()` في `OnboardingPage.tsx` كانت تنتظر ٥ ثوانٍ فقط بعد إغلاق نافذة Meta لاستلام waba_id/phone_number_id عبر postMessage منفصل عن استجابة FB.login نفسها — تحت شبكة أبطأ تصل هذه الرسالة متأخرة فيفشل استدعاء `/embedded-signup/complete` صامتاً رغم نجاح الموافقة الفعلية على مستوى Meta. وُسِّعت المهلة إلى ٢٠ ثانية (لا يمكن اختبارها حياً محلياً — لا إعدادات Meta حقيقية محلياً). **العميل رفض إعادة المحاولة وانسحب فعلياً — خسارة مؤكَّدة، لكن الإصلاح يمنع تكرارها.** بالتوازي: عطل طارئ منفصل ظهر أولاً (خُيِّل أنه السبب): `channel_accounts.default_agent_id` غائب من `migrate-phase345.sql` رغم وجوده في سكيمة Drizzle — أُضيف للملف (نظافة/تناسق)، **لكن تبيّن لاحقاً عبر فحص الإنتاج المباشر أن العمود كان موجوداً بالفعل في قاعدة الإنتاج قبل هذا الإصلاح** (على الأغلب رُقِّع يدوياً سابقاً دون تحديث الملف) — أي أن هذا الإصلاح لم يكن سبب مشكلة هذا العميل تحديداً، درس مهم: لا تفترض تطابق الإنتاج مع الملف المحلي دون تحقق مباشر. + من نفس التحقيق: بوابة `requireVerifiedEmail` تحجب onboarding عن أي تسجيل بكلمة مرور غير مؤكَّد بريده (صفحة onboarding الوحيدة خارج `<Layout>`) — أُضيفت شاشة تحقق + أُصلح عطل جلسة منفصل في `/auth/me` (emailVerified لا يُحدَّث بالجلسة) + عطل تكدّس أزرار الجوال بثلاث خطوات onboarding. جاهز للدفع)
