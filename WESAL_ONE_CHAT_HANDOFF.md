...
**✅ PD-7** (`9cbf5a7`). **✅ PD-8** (`a3cb4a6`، parser ثلاثي). **✅ PD-9 مدفوع** (`e091e4c`، `coerceCurrency`) — لم يُؤكَّد حيّاً بعد لأنه محجوب بـPD-10. **🟡 PD-10 المرحلة 1 مُصلَحة بالكود** (فرض JSON عند تفعيل الأدوات — typecheck نظيف، تنتظر push) — المرحلة 2 (function calling) مؤجلة. **🔴 PD-11 موثّق غير محلول** (لا زر "إعادة للوكيل" — كل تصعيد يَعلَق بشرياً).
...
## 🔴 PD-11 (موثّق، غير محلول) — لا طريقة لإرجاع محادثة للوكيل

أزرار الوارد ("تم الحل"/"إعادة فتح") تعدّل `status`/`needs_human` فقط، **لا `agent_status`** (الحقل الوحيد الذي يفحصه الـworker). فأي محادثة تتصعّد لبشري تَعلَق للأبد. الإصلاح: زر "إعادة للوكيل" يستدعي `PATCH /:id/agent-status` بـ`status=active` (المسار سليم وموجود). **النطاق 26 (CRM/الوارد التشغيلي).**
