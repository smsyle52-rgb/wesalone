...
آخر تحديث: 22 يونيو 2026 (جلسة Claude Code — قنوات ميتا PD-4/5/6، صقل الوكلاء، تصليب، فحص المدفوعات)
...
## 📌 جلسة 22 يونيو 2026 (Claude Code)
- **PD-6 ✅** إنستغرام/ماسنجر: استقبال (إصلاح صيغة `messaging[]`) + إرسال (إنستغرام عبر `{pageId}/messages` — إصلاح Meta `(#3)`) + التسجيل المضمّن موصول.
- **PD-4 ✅** وسائط المنتجات: `send_product_media` يقرأ `inventory_products` ويوجّه حسب القناة.
- **PD-5 (MVP) ✅** مكالمات واتساب: تسجيل + تصعيد + ردّ نصي. خامل حتى تفعيل Calling + اشتراك `calls`.
- **نطاق 11 ✅** (get_order_status) · **صقل الوكلاء ✅** · **cleanup-domain-events 500 ✅** (created_at) · **أسماء العملاء** من ميتا.
- **نطاق 19 (تصليب آمن) ✅** CSP (Report-Only) + تعطيل X-Powered-By. فرض CSP بعد تحقّق التقارير؛ تحديث تبعيات البناء + مفتاح تشفير مستقل = مهام موثّقة.
- **نطاق 12 (المدفوعات) ✅** فحص read-only: تأكيد/رفض يدوي بصلاحية + معاملة ذرّية + حارس دفع زائد + عزل + تدقيق critical؛ أداة الوكيل pending فقط. لا تأكيد آلي، لا ثغرات — بلا تعديل كود.
- **بانتظار المالك (ميتا):** تفعيل WhatsApp Calling + اشتراك `calls`؛ App Review للعموم لاحقاً.
- الدروس التقنية الدائمة في ذاكرة كلود: `wesal-meta-channels-integration`.
