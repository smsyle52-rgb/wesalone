...
import { StopBroadcastDialog } from "@/features/broadcasts/components/stop-broadcast-dialog"
...
vi.mock("@/features/broadcasts/actions/stop-broadcast.action", () => ({
...
describe("StopBroadcastDialog", () => {
  test("binds stopBroadcastAction to (null, workspaceId, broadcastId)", () => {
    renderDialog()

    expect(bind).toHaveBeenCalledWith(null, "ws-1", "bc-1")
  })

  test("confirming calls the bound stopBroadcastAction action", () => {
    const el = renderDialog()
    const confirmButton = Array.from(el.querySelectorAll("button")).find(
      (btn) => btn.textContent === "actions.stop",
    )
  ...
  test("success shows a toast and refreshes so the row moves to cancelled", () => {
    renderDialog()
  ...
  test("a race error (e.g. the broadcast already finished sending) shows the server message and refreshes the stale row", () => {
    renderDialog()

    act(() => {
      actionState.options.onError?.({
        error: { serverError: "Broadcast is no longer sending" },
      })
    })

    expect(toast.error).toHaveBeenCalledWith("Broadcast is no longer sending")
    expect(refresh).toHaveBeenCalledTimes(1)
  })
})
