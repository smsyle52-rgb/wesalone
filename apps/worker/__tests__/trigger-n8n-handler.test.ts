...
const { handleTriggerN8nStep } = await import(
  "../src/integration/handlers/trigger-n8n-handler"
)
...
const createProps = () =>
  ...
  }) as Parameters<typeof handleTriggerN8nStep>[0]
...
describe("handleTriggerN8nStep", () => {
  test("is a no-op when no webhook is registered for the event", async () => {
    await expect(handleTriggerN8nStep(createProps())).resolves.toEqual({
      status: "success",
      result: null,
    })
  ...
  test("posts to every registered webhook and succeeds on 2xx", async () => {
    mocks.listByWorkspaceAndEvents.mockResolvedValue([
      { event: "order_confirmed", url: "https://n8n.example.com/webhook/a" },
      { event: "order_confirmed", url: "https://n8n.example.com/webhook/b" },
    ])
  ...
  test("returns an error state when a webhook responds with 4xx/5xx", async () => {
    ...
    await expect(handleTriggerN8nStep(createProps())).resolves.toMatchObject({
      status: "error",
      errorMessage: expect.stringContaining("order_confirmed"),
    })
  })
