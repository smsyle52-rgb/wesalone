...
export type ReplyByAIExecutionResult = {
  responded: boolean
  provider: AIAgentProvider | "openaiCompatible" | "vertex"
  modelId: string
...
export async function runAIAgentRunner(
  ...
  // Platform-locked Vertex setting takes precedence over anything stored on
  // the agent (and over a flow step's explicit preferredModel/preferredProvider)
  // — see packages/ai/src/server/platform-provider.ts. `null` means disabled,
  // so every line below this behaves exactly as it did before this setting existed.
  const platformOverride = await getActivePlatformAiOverride()

  let providersToRun: (AIAgentModelConfig | PlatformVertexModelCandidate)[]
  if (platformOverride) {
    providersToRun = buildPlatformOverrideCandidates(platformOverride)
  } else {
...
async function createAgentModel(props: {
  ...
  if (isPlatformVertexModelCandidate(providerInfo)) {
    ...
    return getPlatformVertexChatModel(providerInfo.model, override)
  }
