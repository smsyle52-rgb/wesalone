...
const InventoryLevel = model
  .define("InventoryLevel", {
    ...
    inventory_item: model.belongsTo(() => InventoryItem, { mappedBy: "location_levels" }),
    available_quantity: model.bigNumber().computed(),
  })
  .indexes([
    { name: "IDX_inventory_level_inventory_item_id", on: ["inventory_item_id"], where: "deleted_at IS NULL" },
    { name: "IDX_inventory_level_location_id", on: ["location_id"], where: "deleted_at IS NULL" },
    {
