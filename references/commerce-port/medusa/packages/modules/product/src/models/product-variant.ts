...
// Source: medusajs/medusa@7686adc8637b41879c4551ca422829ad2d63d6a9
...
const ProductVariant = model
  .define("ProductVariant", {
    ...
    product: model.belongsTo(() => Product, { mappedBy: "variants" }).searchable().nullable(),
  ...
  .indexes([
    { name: "IDX_product_variant_id_product_id", on: ["id", "product_id"], where: "deleted_at IS NULL" },
    { name: "IDX_product_variant_product_id", on: ["product_id"], where: "deleted_at IS NULL" },
    { name: "IDX_product_variant_sku_unique", on: ["sku"], unique: true, where: "deleted_at IS NULL" },
    { name: "IDX_product_variant_barcode_unique", on: ["barcode"], unique: true, where: "deleted_at IS NULL" },
    { name: "IDX_product_variant_ean_unique", on: ["ean"], unique: true, where: "deleted_at IS NULL" },
    { name: "IDX_product_variant_upc_unique", on: ["upc"], unique: true, where: "deleted_at IS NULL" },
  ])
