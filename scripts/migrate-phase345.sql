...
VALUES
  ...
  -- إكماله مستحيلاً فعلياً على المجاني. monthly_points خُفِّضت 1000→500 (6 يوليو، تسجيلات
  -- جديدة فقط — القديمة تحتفظ بمنحتها الأصلية عبر point_grants ثابت لا يتأثر بهذا التغيير).
  ('free','free','Free','مجاني', 0, 0, 0, NULL, 'monthly', true, 10,
    '{"channels":2,"agents":1,"team_members":1,"contacts":100,"monthly_points":500,"knowledge_documents":1,"products":20,"auto_reply":false}'::jsonb,
...
-- 0031_billing_admin_manual_payment_closure
-- Subscription manual payments are reviewed only by platform admins, with
-- immutable server-side price snapshots and one pending review per plan/cycle.
ALTER TABLE subscriptions ADD COLUMN IF NOT EXISTS billing_cycle text NOT NULL DEFAULT 'monthly';
ALTER TABLE payment_submissions ADD COLUMN IF NOT EXISTS billing_cycle text NOT NULL DEFAULT 'monthly';
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'chk_payment_submissions_status_lifecycle'
  ) THEN
    ALTER TABLE payment_submissions
      ADD CONSTRAINT chk_payment_submissions_status_lifecycle
      CHECK (status IN (
        'pending','payment_submitted','pending_payment','under_review',
        'confirmed','approved','rejected','cancelled','expired','refunded','chargeback'
      ));
  END IF;
END $$;
...
-- 0032_whatsapp_bsuid_identities — scoped WhatsApp customer identities (phone + BSUID)
-- Additive only: keeps contacts.phone and existing contact_channels intact while
-- allowing WhatsApp Business-Scoped User IDs to map to the internal contact_id.
...
-- ── 0034_backfill_channel_external_ids (W6-T1) ────────────────────────────────
-- Backfill channel_accounts.external_{account,business,phone}_id from each row's
-- own provider_config jsonb. Self-contained per row — no cross-table matching,
-- no cross-tenant risk. Only fills NULLs; never overwrites a live-set value.
...
-- ── 0035_event_subscriber_progress (W4-T3) ────────────────────────────────────
-- Per-subscriber idempotent progress tracking for the event-dispatcher.
...
-- ── 0037_messages_reply_to (W3-T2) ────────────────────────────────────────────
-- Reply/quote threading. No FK — the quoted message may be pruned
...
-- 0039_meta_mobile_signup_attempts
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_meta_mobile_signup_attempt_id ON meta_mobile_signup_attempts(signup_attempt_id);
CREATE UNIQUE INDEX IF NOT EXISTS uq_meta_mobile_signup_nonce_hash ON meta_mobile_signup_attempts(nonce_hash);
CREATE INDEX IF NOT EXISTS idx_meta_mobile_signup_workspace_status ON meta_mobile_signup_attempts(workspace_id, status);
CREATE INDEX IF NOT EXISTS idx_meta_mobile_signup_lease ON meta_mobile_signup_attempts(lease_expires_at);
CREATE INDEX IF NOT EXISTS idx_meta_mobile_signup_expires ON meta_mobile_signup_attempts(expires_at);
